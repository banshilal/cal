...
$pageFlip_class_php = dirname(__FILE__).'/pageFlip.class.php';
include_once $pageFlip_class_php;


if (class_exists('pageFlip_plugin_base'))
{
	...
	$pageFlipUpdate = new PageFlipUpdate('http://pageflipgallery.com/update/', 'stable', basename(dirname(__FILE__)), basename(__FILE__));
	add_filter('pre_set_site_transient_update_plugins', array($pageFlipUpdate, 'check_for_plugin_update'));
	add_filter('plugins_api', array($pageFlipUpdate, 'plugins_api_call'), 11, 3);
}
